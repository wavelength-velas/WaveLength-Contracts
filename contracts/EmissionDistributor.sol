    ...
    AccessControl
{
    ...
    struct TokenInfo {
        // we have a fixed number of WAVE tokens released per block, each pool gets his fraction based on the allocPoint
        address user; 
        uint256 numberNFT; // How many allocation points assigned to this pool. the fraction WAVE to distribute per block.
    }

    mapping(uint256 => mapping(address => TokenInfo)) public tokenInfo; // mapping form poolId => user Address => User Info
    ...
    function setLocker(FBeetsLocker fwaveLocker)
        external
    ...
    function depositToChef(uint256 _tokenId) external {
    ...
    function _distribute() internal {
        if (wave.balanceOf(address(this)) > 0) {
            ...
            if (fBeetsLockerShare > 0) {
                ...
                uint256 rewardsForLockedFBeets = (wave.balanceOf(
                    address(this)
                ) * fBeetsLockerShare) / DENOMINATOR;
                wave.approve(address(locker), rewardsForLockedFBeets);
            ...
            if (fBeetsLockerShare < DENOMINATOR) {
                ...
                // now we take the resulting fidelioDuetteBpt's and share them via revenue to the wave bar
                fidelioDuetteBpt.approve(
                    address(waveBar),
                    fidelioDuetteBpt.balanceOf(address(this))
                );

                waveBar.shareRevenue(
                    fidelioDuetteBpt.balanceOf(address(this))
                );
            }
        }
