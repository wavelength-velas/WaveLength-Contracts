...
contract CompoundLinearPool is LinearPool {
    ICToken private immutable _tokenVault;
    ...
    {
        _tokenVault = ICToken(address(wrappedToken));
        _require(address(mainToken) == ICToken(address(wrappedToken)).underlying(), Errors.TOKENS_MISMATCH);
        require(ICToken(address(wrappedToken)).isCToken() == true, "WrappedToken is not CToken");
    }
    ...
    function _getWrappedTokenRate() internal view override returns (uint256) {
        ...
        uint256 pps = _tokenVault.exchangeRateStored();
...
interface ICToken {
    /**
     * @dev returns the address of the Compound Token's underlying asset (mainToken)
     */
    function underlying() external view returns (address);

    /**
     * @dev returns the current exchange rate for this Compound Token
     */
    function exchangeRateStored() external view returns (uint256);

    /**
     * @dev returns the number of decimals for this interest-bearing Compound token
     */
    function decimals() external view returns (uint8);

    /**
     * @dev returns true if token is CToken and false if not
     */
